...
def _get_path(dataset, model, tag):
    ...
    return 'models/'+ dataset + model + tag + '.model'    
...
def eval_tta(config, augment, reporter):
    ...
    #print ('report',metrics['minus_loss'],metrics['correct'],gpu_secs)
...
if __name__ == '__main__':
    ...
    num_result_per_cv = 10 #top 10 policies
    cv_num = 5
    ...
    #paths = [_get_path(C.get()['dataset'], C.get()['model']['type'], 'fold%d' %  (i) ) for i in range(cv_num)]
    
    '''
    print(paths)
    reqs = [
        train_model.remote(copy.deepcopy(copied_c), args.dataroot, C.get()['aug'], args.cv_ratio, i, save_path=paths[i], skip_exist=False)
    ...
    def eval_t(augs, reporter):
        print(augs)
        return eval_tta(copy.deepcopy(copied_c), augs, reporter)
    ...
    for _ in range(1):  # run multiple times.
        for cv_fold in range(cv_num):
            ...
            print(name)
            algo = HyperOptSearch(space, mode = "max", max_concurrent=1, metric=reward_attr)
            aug_config = {
                'dataroot': '/media/zwan074/Untitled/fast-autoaugment-master/data/private/pretrainedmodels', 'save_path': '/media/zwan074/Untitled/fast-autoaugment-master/' + paths[cv_fold],
                'cv_ratio_test': args.cv_ratio, 'cv_fold': cv_fold,
                'num_op': args.num_op, 'num_policy': args.num_policy
            }
            num_samples = 2 #if args.smoke_test else args.num_search
            ...
            print(results)
            #print(dataframe.to_csv('test.csv'))
